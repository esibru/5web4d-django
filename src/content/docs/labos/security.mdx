...
``` python showLineNumbers=false title=developer/views.py ins={1,6,11} del={5,10}
...
``` python showLineNumbers=false title=task/views.py ins={1,6} del={5}
  from django.contrib.auth.mixins import LoginRequiredMixin
...
``` python showLineNumbers=false title=task/models.py
...
```python showLineNumbers=false title=task/views.py del={1,6} ins={2,7,10}
  from django.contrib.auth.mixins import LoginRequiredMixin
...
```html showLineNumbers=false title=task/_create_task_modal.html
